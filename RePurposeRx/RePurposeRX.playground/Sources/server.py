...
WSGIRequestHandler.timeout = 3000 

def process_data_concurrently(metadata):
    """Process all data sources concurrently using ThreadPoolExecutor"""
    with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
        # Submit all tasks
        drug_future = executor.submit(mega.process_drug_data, metadata)
        gene_future = executor.submit(mega.process_gene_data, metadata)
        disease_future = executor.submit(mega.process_disease_data, metadata)
        clinical_trial_future = executor.submit(mega.process_clinical_trial_data, metadata)
        literature_future = executor.submit(mega.process_literature_data, metadata)
        omics_future = executor.submit(mega.process_omics_data, metadata)
        fda_future = executor.submit(mega.process_fda_data, metadata)
...
def deepseek_query():
    ...
    try:
        # Extract metadata
        metadata = mega.extract_metadata(query)
        ...
        # Generate response
        deepseek_response = mega.generate_response(
            query, metadata
        )
